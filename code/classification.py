...
#                 self.pp("show_models: " + str(automl.show_models()))
    ...
    def ten_fold(self,X,y,model='sk',max_memory=1024*32,oversampling=False,max_time=60*60,n_jobs=1):
        output_df = pd.DataFrame(columns=['index','round','precision','recall','f1-score','support'])
        roc = list()
...
#                 self.pp("show_models: " + str(automl.show_models()))
                ...
                classification_output.append((runner, test_index, y_hat, y_test))
                runner += 1
        finally:
            for row,col in output_df.groupby('index').mean().iterrows():
                col['index'] = row + " mean"
            ...
            index_output = list()
            for runner, test_index, y_hat, y_test in classification_output:
                y_hat = y_hat.tolist()
                y_test = y_test.tolist()
                for i, test_i in enumerate(test_index):
                    index_output.append((runner, test_i, y_hat[i], y_test[i]))

            pd.DataFrame(index_output, columns=['round','index', 'y_hat', 'y_test']).to_csv(self.filename+"_index.csv", index=False)
